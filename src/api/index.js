...
//获取热门歌单
export const reqHotSongList = (num) => ajax(`api/personalized?limit=${num}`)
//获取新碟上架
...
//获取歌单详情
export const reqPlaylistDetail = (id) => ajax(`api/playlist/detail?id=${id}`)
// 获取歌单所有歌曲
export const reqplaylistsong = (id,limit,offset) => offset ? ajax(`api/playlist/track/all?id=${id}&limit=${limit}&offset=${offset}`) : ajax(`api/playlist/track/all?id=${id}&limit=${limit}`)
//添加或删除歌单歌曲
...
// 发送验证码
export const sendAuth = (phone) => ajax(`api/captcha/sent?phone=${phone}`)
// 提交验证码
export const submitAuth = (phone,auth) => ajax(`api//captcha/verify?phone=${phone}&captcha=${auth}`)
//检测手机号是否已注册
...
// 听歌次数增加
export const scrobble = (data) => ajax(`api/scrobble?id=${data.id}&sourceid=${data.sourceid}&time=${data.time||'180'}&timestamp=${data.timestamp}`)