<template>
    <div id="video-related">
        <ul class="related-list" v-if="videos.length>=5">
            <li class="related-item flex" :class="{'my-card':$route.name === 'video-player'}"
              ...
              <div class="info">
                ...
                <div class="name" v-if="video.artists&&video.artists.length">
                        <a :href="`/music/singer/${art.id}`"
                           class="text-hv"
                           v-for="(art,index) in video.artists"
                           :key="index">
                            {{art.name}}
                    ...
                    <div class="name" v-else>
                        <a
                                ...
                                class="text-hv"
                                v-for="(user,index) in video.creator"
                                :key="user.userId"
                                title="查看主页"
                        >
                            {{user.nickname}}
                    ...
                    <p class="play-count" :title="`播放次数:${video.playTime || video.playCount}`">播放次数:{{methods.playCount(video.playTime || video.playCount)}}</p>
                </div>
            </li>
        </ul>
        <ul class="related-list related-ske" v-else>
          <li v-for="index in 8" :key="index" class="flex related-item"
