<template>
  <div id="search">
    ...
    <div class="search-result-content container">
      ...
      <template v-if="result.hlWords !== null">
        <template v-if="$route.query.type==='1'&&result!==0">
          <song-list :songs="result.songs||[]" v-if="songlistShow"/>
        </template>
        <template v-else-if="$route.query.type==='100'&&result.artistCount!==0">
          <singer-list :singers="result.artists||[]"/>
        </template>
        <template v-else-if="$route.query.type==='10'&&result!==0">
          <album :albums="result.albums||[]"/>
        </template>
        <template v-else-if="$route.query.type==='1000'&&result!==0">
          <PlayList :playlists="result.playlists||[]"/>
        </template>
        <template v-else-if="$route.query.type==='1002'&&result.userprofiles">
          <userList :users="result.userprofiles||[]"/>
        </template>
        <template v-else-if="$route.query.type==='1004'&&result!==0">
          <mv-list :skeCount="15" :mvs="result.mvs||[]"/>
        </template>
        <template v-else-if="$route.query.type==='1006'&&result.songs">
          <lyric-list :songs="result.songs"/>
        </template>
        <template v-else-if="$route.query.type==='1009'&&result.djRadios">
          <dj-list :djs="result.djRadios"/>
        </template>
        <template v-else-if="$route.query.type==='1014'&&result!==0">
          <video-list :skeCount="15" :videos="result.videos||[]"/>
        </template>
