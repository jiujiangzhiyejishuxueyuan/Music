<template>
  <div class="home-rank container my-card recommend-item">
    <div class="">
...
<script>
import {reqLikeSong, reqPlaylistDetail} from "@/api";
...
export default {
  ...
  methods: {
    play(index) {
      this.methods.playMusic(this.ids[index])
    },
    render() {
      if (!this.list?.length) {
        const list = [19723756, 3779629, 3778678]
        list.forEach((item, index) => {
          reqPlaylistDetail(item).then(res => {
