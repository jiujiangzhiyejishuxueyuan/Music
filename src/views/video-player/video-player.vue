  ...
  <div class="video-player">
    <div class="video-player-header container flex justify-between align-center my-card" v-if="video.title">
      <div class="info">
        ...
        <p class="time" v-if="video.publishTime">发布时间:{{  methods.dataFormat(video.publishTime) }}</p>
        <p class="count">
          <span class="play-count">播放次数:{{  methods.playCount(video.playTime) }}</span>
  ...
  created() {
    ...
    reqVideoList(58100).then(res => {
      res.datas.forEach(item => {
        ...
        this.relatedVideos.push(item.data)
      })
    })
