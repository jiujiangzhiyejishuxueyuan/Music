...
import {reqLoginState, reqUserInfo,follow} from "@/api";
...
function render() {
  ...
  reqUserInfo(id).then(res => {
    ...
    user.value = res
    user.value.bindings.forEach(item => {
      if (item.url) {
        user.value.bindings = item
      }
    })
  }).catch(() => router.replace({name: '404'}))
}
function goFollow(t) {
  follow(this.user.profile.userId, t).then(() => {
    user.value.profile.followed = !user.value.profile.followed
  })
}
const birthday = computed(() => {
  let year = new Date(user.value.profile.birthday).getFullYear()
  year = year % 100
  if (year % 10 >= 5) {
    ...
    <div class="header-info">
      ...
      <div class="container header-inner flex my-card">
        <div class="avatar-box" @click="$router.replace(`/user/home?id=${user.profile&&user.profile.userId}`)">
          <img :src="user.profile&&user.profile.avatarUrl" alt="头像" title="主页">
        </div>
        <div class="info-inner" v-if="user.profile">
          <div class="info-inner-header flex align-center">
            ...
            <div class="control flex" v-if="userInfo&&(userInfo.userId!==user.profile.userId)">
              ...
              <span class="follow item" v-if="!user.profile.followed" @click="goFollow(1)">关注</span>
              <span class="followed item" v-else @click="follow(0)"><span class="t">已关注</span> <span
                  class="t-h">取消关注</span></span>
            </div>
          ...
          <div class="area" v-if="user.profile.province">
            地区:
            {{ areaList.province_list[user.profile.province] }}-{{ areaList.city_list[user.profile.city] ? areaList.city_list[user.profile.city] : areaList.county_list[user.profile.city] }}
